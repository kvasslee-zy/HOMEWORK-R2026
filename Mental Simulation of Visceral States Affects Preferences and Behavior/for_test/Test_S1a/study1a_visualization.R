...
out_dir <- "C:/Users/lenovo/Desktop/R0507/Mental Simulation of Visceral States Affects Preferences and Behavior/S1a"
...
# Load data
# --------------------------------------------------------------------------
df <- read_spss("C:/Users/lenovo/Desktop/R0507/Mental Simulation of Visceral States Affects Preferences and Behavior/osfstorage-archive (1)/Study1/S1.sav")
...
# ===========================================================================
# TABLE 5: COMPARISON WITH ORIGINAL PAPER
# ===========================================================================
cat("Generating Table 5: Comparison with Original Paper...\n")

t5_data <- data.frame(
  Metric = c("Sample Size (N)", "Cronbach's α", 
             "Hot Condition: M (SD)", "Cold Condition: M (SD)",
             "t-value", "df", "p-value", "Cohen's d",
             "Manipulation Check: t", "Manipulation Check: p",
             "Conclusion"),
  Original = c("119", ".64", "—", "—", "—", "—", 
               "< .05", "~0.55 (power est.)", "—", "—",
               "Simulating warmth ↓\npreference for warming"),
  Replication = c(
    sprintf("%d", n1 + n2), sprintf("%.2f", alpha),
...
t5 <- make_table(t5_data, header_bg = COL_YELLOW,
                 title = "Table 5: Comparison — Original Paper vs. Current Replication",
                 fontsize = 10)
save_grob_png(t5, "Table5_Comparison.png", width = 10, height = 5)
